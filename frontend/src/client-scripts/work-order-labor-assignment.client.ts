...
export const formOnLoad = ({ form, setFieldQuery }: FormContext) => {
  ...
  if (form.work_order_labor || start_datetime || end_datetime) {
    setFieldQuery?.('equipment', {
      query: 'ci_eam.work_management.doctype.work_order_labor_assignment.work_order_labor_assignment.search_employee_query',
      ignore_user_permissions: false,
      page_length: 20,
      filters: {
        work_order_labor: form.work_order_labor || '',
...
export const formFieldChange = ({ form, fieldname, setFieldQuery, notify }: FormContext) => {
  ...
  if (fieldname && ['work_order_labor', 'start_datetime', 'end_datetime', 'start_datetime_range'].includes(fieldname)) {
    const { start_datetime, end_datetime } = getDatetimeValues(form)
    
    // Check if required fields are missing and notify user
    const missingFields = []
    if (!form.work_order_labor) missingFields.push('Work Order Labor')
    if (!start_datetime) missingFields.push('Start Datetime')
    if (!end_datetime) missingFields.push('End Datetime')

    if (missingFields.length > 0) {
      notify.warning(`Please fill in the following fields to filter labor: ${missingFields.join(', ')}`)
    }

    setFieldQuery?.('employee', {
      query: 'ci_eam.work_management.doctype.work_order_labor_assignment.work_order_labor_assignment.search_employee_query',
      ignore_user_permissions: false,
      page_length: 20,
      filters: {
...
export const formAfterDelete = ({ redirect }: FormContext) => {
  redirect?.('/work-order-labor-assignment')
} 